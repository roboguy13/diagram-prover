...
export type PortContext = Map<string, PortLocation>

export function termToStringDiagram(term: Term): StringDiagram {
  switch (term.type) {
    case 'Var':
      return varTermToDiagram(term);
    case 'Lam':
      return lamTermToDiagram(term);
    case 'App':
      return appTermToDiagram(term);
    case 'unit':
      return unitTermToDiagram();
    default:
      throw new Error(`Unhandled term type: ${term.type}`);
  }
}

function varTermToDiagram(term: VarTerm): StringDiagram {
}
